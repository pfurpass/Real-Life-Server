...
public class StreamDestinationCommandsTests
{
    ...
    [Fact]
    public async Task Add_PersistsAsANewRow_NotAnUpdateToANonExistentOne()
    {
        // Regression test for the reported production bug: `channel.Destinations.Add(new
        // StreamDestination {...})` tracked the new entity as Modified, not Added - because
        // StreamDestination.Id is a client-generated, non-default Guid and Npgsql's EF Core
        // provider configures Guid keys as store-generated by convention, EF's DetectChanges
        // graph-walk assumed the entity already existed and generated an UPDATE matching zero
        // rows (DbUpdateConcurrencyException: "expected to affect 1 row(s), but actually affected
        // 0 row(s)"). A hand-written IApplicationDbContext fake can't catch this - it has no real
        // change-tracking semantics - so this uses EF Core's InMemory provider, which enforces the
        // same Added/Modified state and row-matching rules as a relational provider. Reloading via
        // a brand new DbContext instance (not just checking SaveChangesAsync didn't throw) proves
        ...
        var handler = new AddStreamDestinationCommandHandler(
            new InMemoryChannelRepository(db), db, new FakeEncryptionService(), new FakeStreamOrchestrator());

        await handler.Handle(
            new AddStreamDestinationCommand(channel.Id, StreamPlatform.Twitch, "rtmp://live.twitch.tv/app", "key"),
            CancellationToken.None);

        await using var verifyDb = CreateInMemoryDb(dbName);
        var persisted = await verifyDb.StreamDestinations.SingleAsync();
        Assert.Equal(channel.Id, persisted.ChannelId);
        Assert.Equal("rtmp://live.twitch.tv/app", persisted.RtmpUrl);
    }
    ...
    public async Task Add_ValidDestination_EncryptsKeyAndNeverStoresPlainText()
    {
        ...
        var stored = await db.StreamDestinations.SingleAsync();
    ...
    public async Task Add_RestartsTheCompositor_SinceNewDestinationsStartEnabled()
    {
        ...
        var handler = new AddStreamDestinationCommandHandler(new InMemoryChannelRepository(db), db, new FakeEncryptionService(), orchestrator);
    ...
    /// <summary>
    /// Real EF Core (InMemory provider) instead of a hand-written fake, specifically for the Add
    /// tests: they need genuine change-tracking/Added-vs-Modified semantics, which a dictionary-
    /// backed fake cannot reproduce (see Add_PersistsAsANewRow_NotAnUpdateToANonExistentOne).
    /// </summary>
    private sealed class TestDbContext(DbContextOptions<TestDbContext> options) : DbContext(options), IApplicationDbContext
    {
    ...
    private static TestDbContext CreateInMemoryDb(string name) =>
        new(new DbContextOptionsBuilder<TestDbContext>().UseInMemoryDatabase(name).Options);

    private sealed class InMemoryChannelRepository(TestDbContext db) : IChannelRepository
    {
        public Task<Channel?> GetByIdAsync(Guid id, CancellationToken ct = default) =>
            db.Channels.Include(c => c.Destinations).FirstOrDefaultAsync(c => c.Id == id, ct);
        public Task<Channel?> GetByStreamKeyAsync(string streamKey, CancellationToken ct = default) => throw new NotSupportedException();
        public Task<Channel?> GetBySlugAsync(string slug, CancellationToken ct = default) => throw new NotSupportedException();
        public Task<List<Channel>> GetForOwnerAsync(Guid ownerUserId, CancellationToken ct = default) => throw new NotSupportedException();
        public Task<List<Channel>> GetAllAsync(CancellationToken ct = default) => throw new NotSupportedException();
